...
\begin{defn}\label{def_pre_cycmod}
  Let $X$ be a finite-type $k$-scheme, and let $\fields(X)$ be the
  class of all fields $E$ such that there exists a map $\Spec E \to
  X$, and that $E$ is finitely generated over $k$.
...
\begin{defn}
  A cycle module $M$ on $\fields(X)$ is a cycle premodule that
