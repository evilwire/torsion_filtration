...
The suspension $\LHI{\tlHM{n}(F_*)_{k - 1}} \to 
\tlHM{n}(F_*)_{k}$ is given by composing the isomorphism
\[
...
\]
It is straight-forward to verify that if $\LHI{F_{k - 1}} \to 
F_{k}$ is the suspension map for which its adjunction 
$\RHI{(F_{k})} \to F_{k - 1}$ is the isomorphism given above,

Functoriality of $\tlHM{n}$ follows from the naturality of the
isomorphisms above (Prop. \ref{prop_tl_L_R}).
