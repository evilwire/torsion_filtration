...
\end{prop}
\begin{proof}
Let $F$ be an object in $\tgHI{n}\HI$, and $G$ be an object in 
$\tgHI{m}\HI$. Then the counit $\epsilon: L^nR^n(F) \to F$ is 
surjective (Prop.  \ref{prop_THI_properties} (1)), and since the 
functor $- \tHI G$ is right exact, the map
\[
