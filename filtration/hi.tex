...
\end{equation}
Fix any $C_1, C_3$ in $\Cat{C}$ and $C_2$ in $\Cat{C} \cap 
\DCat^{\compact}$ such that $\ihom(C_2, -)$ is left $t$-exact (i.e.
takes objects in $\DCat^{\geq 0}$ to objects in $\DCat^{\geq 0}$).
Since $\C_1 \tensor C_2$ is in $\DCat^{\leq 0}$, using the 
isomorphism in \eqref{eq_t_struct_hom_eq_1}, we obtain the 
following:
\begin{align*}
\hom_{\Cat{C}}(C_1 \tensor^{\Cat{C}} C_2, C_3) 
\hom(\HH^0(C_1 \tensor C_2), C_3) \\
   &\cong \hom_{\DCat{C}}(C_1 \tensor C_2, C_3)
\end{align*}
Since $- \tensor C_2$ is adjoint to $\ihom(C_2, -)$ and 
$\ihom(C_2, C_3)$ is in $\DCat^{\geq 0}$, by 
\eqref{eq_t_struct_hom_eq_2}, we obtain the following
\begin{align*}
\hom_{\Cat{C}}(C_1 \tensor^{\Cat{C}} C_2, C_3)
&\cong \hom_{\DCat}(C_1 \tensor C_2, C_3) \\
...
internal hom where the collection of semi-representable objects of
...
category $\HI$ as its heart (see \cite[5.2]{DegModHom}). We write 
$\gltrunc{0}\DMeff$ for the negative objects of $\DMeff$, and 
...
and $\HI$, respectively. Moreover, the triangulated monoidal 
...
Since $m > n$, by Lemma \ref{lem_HH_commutes_with_contract} 
