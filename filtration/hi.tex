...
two descending and one ascending --- on the category $\HI$: one 
related to the slice filtration on $\HI$, and the another one 
obtained from a sequence of coradicals. The main result in this 
section is that the latter filtration form a strong filtration 
(see \ref{def_strong_filtration} below). The key ingredient in the 
constructions of both filtration is the tensor monoidal structure 
on $\HI$, together with a partial internal hom induced by the 
tensor and partial internal hom operators on $\DMeff$. 
...
$\HH^n : \DCat \to \Cat{C}$. This is

\begin{thm}[\cite{BBD} Thm. 1.3.6]\label{thm_heart_is_abel_cat}
Let $\DCat$ be a $t$-category, equipped with a $t$-structure 
$(\DCat^{\geq 0}, \DCat^{\leq 0})$. Then the heart $\Cat{C}$ is an 
admissible abelian category. 
\end{thm}
...
abelian except for $n = 0$. To see this, consider the map
\[
n: \Ox \to \Ox
\]
...
subobject of $A$. A descending filtration $(\Cat{A}, \gamma_*)$ 
...
There exists a descending strong filtration $(\tgHI{n}\HI, 
\tgHI{n})$ and an ascending strong cofiltration $(\tlHI{n}\HI, 
\tlHI{n})$ of the category $\HI$ defined by a sequence of 
coradicals.
\end{thm}
...
We first define the strong cofiltration. Let $n$ be a natural 
number, and let $\tlHI{n}\HI$ be the full subcategories of objects 
$F$ in $\HI$ such that $\RHI[n]{F} = 0$. Here, we define 
$\RHI[n]{F} \defeq F$ for $n = 0$. It is easy to see that we also 
...
of $\HI$. We need to verify that $\RHI[n]{(\tlHI{n}(F))}$ is 0.
...
The functors $\tlHI{n}$ are coradicals for all natural numbers
$n$.
\end{prop}
\begin{proof}
...
For each natural number $n$, there exists a torsion pair 
\[
(\tgHI{n}\HI, \tlHI{n}\HI)
\] 
...
objects $F$ such that $\sgHI{n}(F) \to F$ is surjective.
\tinyskip
...
\pfitem{(2)} : Let $m$ and $n$ be positive integers such that
$m > n$, and fix $F$ in $\THI{m}$. Then $\tlHI{m}(F) = 0$, and by 
Prop. \ref{prop_TFHI_properties} (3)
\[
...
transfers. In the case where $m \geq n$, the claim follows since 
$\tgHI{m}(F) \in \THI{n}$, and by definition $\tlHI{n} \tgHI{m}(F) 
= 0$ for all $n \leq m$. Furthermore, $\tgHI{m}(F) = 0$ since it 
is the kernel of the $\tlHI{m} \tlHI{n}(F) \to \tlHI{n}(F)$ which 
is the identity map by part (3) of the preceding proposition.
...
filtration $(\DMeff, \sgDM{*})$ and the (weak) cofiltration 
$(\DMeff, \slDM{*})$ (see Sect. \ref{sect_slice_filt_dm}).
...
\simeq \LHI[m]{\RHI[m]{(\LHI[n]{\RHI[n]{F}})}}$. Furthermore, the 
