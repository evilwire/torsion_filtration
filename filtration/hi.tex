...
If in addition $\DCat$ is equipped with an additive symmetric 
...
An important property about the functor $\RHI{?}$ is the following

\begin{prop}[\cite{DegGenMot} or \cite{MVW}]
...
Let $\cuHI^n$ denote the counit that is described above. If $F$ is
an object of $\LHI[n]{\HI}$, then $\cuHI^n : \LHI[n]{\RHI[n]{F}} 
\to F$ is an isomorphism.
\end{prop}
\begin{proof}
Suppose $F$ is in $\LHI[n]{\HI}$. Then, by definition, $F = 
LHI[n]{F'}$ for some $F'$ in $\HI$. Writing $L$ for the functor 
...
denote the full subcategory of objects $F$ where $F = \LHI[n]{F'}$ 
...
by Prop. \ref{prop_unit_iso}, forcing $\Z = 0$. Inductively, we see 
that $\LHI[n]{\HI} \neq \LHI[n + 1]{\HI}$ for all natural numbers 
$n$.
...
of $\HI$. To show that $\tlHI{n}(F) \in \tlHI{n}\HI$, we verify that 
$\RHI[n]{(\tlHI{n}(F))}$ is 0.
...
$F$ be homotopy invariant sheaf with transfers, and let $G$ be an 
object in $\tlHI{n}\HI$. For all $f: F \to G$ we have the 
...
$(\Cat{A}, \nu_*)$ is a \DEF{strong filtration} if for each $A$
in $\Cat{A}$ and $n$ in $\Z$, $\nu_n A$ is a subobject of $A$. 
Similarly a filtration $(\Cat{A}, \gamma_*)$ is a \DEF{strong 
cofiltration} if $\gamma_n A$ is a quotient of $A$ for each $n$.
\end{defn}
