...
wish to consider a ``bread'' analogy. Imagine a half-infinite loaf 
of bread is laid out on a line marked from 0 to $\infty$, and one 
is allowed to take cuts at the marked points, and subsequently 
pick up all the bread lying greater than $n$ or less than
$n$. The former describe the functors $\tgHI{n}$ and $\sgHI{n}$ 
are analogous, and therefore, the higher the $n$, the less bread
one takes; the latter, functors $\tlHI{n}$.
...
structure invariants for $(\HI, \tgHI{*})$. For each $F \in \HI$
and $n \geq 0$ let $\tconst{n}$ denote $\tlHI{n + 1} \tgHI{n} F$,
called that $n$-th structure invariant of $\tgHI{*}$. As it turns 
out, this is \emph{not} the same as the structure invariants of 
$\sgHI{*}$, but we do have the following:
