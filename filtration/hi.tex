...
As for the filtration $(\HI, \sgHI{*})$, we can define the 
...
out, this is the same as the structure invariants of $\sgHI{*}$:

\begin{prop}
Let $m > n > 0$. Then $\tlHI{n} \sgHI{m}$ is naturally isomorphic 
to $\tlHI{n} \tgHI{m}$. In particular, $\slice{n} = \tconst{n}$.
\end{prop}
