...
We say that $\DCat$ is a \DEF{torsion monoidal category} if $\DCat$
is equipped with 
\begin{enumerate}
\item (\itemhead{Partial Internal Hom}) a partial internal hom
structure $(\ihom, \DCat^{\compact})$

\item (\itemhead{Tate Object}) an object $S$, called the 
   \DEF{Tate object}, in $\DCat^{\compact} \cap \Cat{C}$, where 
