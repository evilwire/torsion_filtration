...
The following gives a good characterization of the category 
$\ALocal$.
...
In this case, we call $\DCat^{\compact}$ the \DEF{compact objects} 
of $\DCat$, and $\ihom$ the partial internal hom (bi)functor on
$\DCat$. We call the pair $(\ihom, \DCat^{\compact})$ the \DEF{partial
hom structure on $\DCat$}.
\end{defn}

We will now show that $\DMeff$ is equipped with an additive 
...
\cite[Corollary 4.10]{V02}. Since $\Z(1)$ is an object of 
$\DMeffgm$, $-\tDM \Z(1)$ and $\ihomDMf(\Z(1), -)$ are adjoint 
pairs by Prop. \ref{prop_DMgm_monoidal}. To simplify notations, 
for $M$ in $\DMeff$, we write $\tZ{M}$ for $M \tDM \Z(1)$, and 
...
\tDM \Z(n)$. It follows that $M \mapsto \hZ[n]{M}$ is equal to 
$\ihomDMf(\Z(n), -)$. We write $\LDMf{n}$ for the endofunctor $- 
\tDM \Z(n)$, and $\RDMf{n}$ for $\ihomDMf(\Z(n), -)$, and set 
$\LDMf{0}$ and $\RDMf{0}$ to be the identity functor. By
Proposition \ref{prop_DMgm_monoidal}, $\LDMf{}$ is left adjoint
to $\RDMf{}$. By induction, we have that for each nonnegative 
integer $n$, $(\LDMf{n}, \RDMf{n})$ form a left-right adjoint 
pair.
...
subsequent sections. One important corollary of Theorem
