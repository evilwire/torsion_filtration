...
Let $\Cat{W}_{\A}$ be the thick subcategory of $\DCat^-\ShCor$ 
...
\DEF{$\A^1$ weak equivalence} if $f \in \Cat{S}_{\A}$.
\end{defn}

\begin{lem}[\cite{MVW} Lemma 9.5]
...
\begin{defn}[\cite{BBD}]
Let $\DCat$ be a triangulated category. A \DEF{$t$-structure}
on $\DCat$
\end{defn}
