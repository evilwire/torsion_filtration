...
abelian category $\PST$ as the homotopy category of cochain 
...
The bounded above derived category $\DCat^-\ShCor$ of Nisnevich
sheaves with transfers is obtained from $\DCat^- \PST$ by localizing
...
$\suslC[*] F^*$ is $\suslC[-p] F^q$. Therefore, $\suslC[]F^*$
again represents an object of $\DShCor$.
\end{defn}
...
cochain complex of Nisnevich sheaf with transfers concentrated
...
I^*).
\]
By \cite[3.2.9]{TriCa}, $\HH^k \sheaf{\rhom}(\Ztr(X), B^*) = 0$
for all $k > \dim X + l$, where $l$ is the smallest index such
that $\HH^0B^*$ has non-vanishing cohomolgoy. Hence 
$\sheaf{\rhom}(\Ztr(X), B^*)$ is an object of $\DShCor$. We can 
extend $\sheaf{\rhom}$ in the first factor to the full subcategory 
of $\DShCor$ generated by the sheaves $\Ztr(X)$.
\end{defn}
