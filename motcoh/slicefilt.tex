...
\end{equation}
By \cite[1.4(i, ii)]{HuKa} that $M'$ is uniquely defined up to 
unique isomorphism, and that there is a cohomological functor 
...
\begin{proof}
\begin{enumerate}
\item Apply the commutative diagram of functors 
...
filtration of $\DM$, we must construct an extension of the functors 
$\sgDM{k}$.  By convention, for all $M$ in $\DMeff$, define 
$\sgDM{n} M$ to be $M$ for all $n \leq 0$, and set
\[
...
M' \to M \to M'' \to M'[1]
\]
Since $\sgDM{k - n}$ is a triangulated functor on $\DMeff$, it
follows that
\[
\sgDM{k - n}M' \to \sgDM{k - n}M \to \sgDM{k - n}M'' \to
\sgDM{k - n}M'[1]
\]
is distinguished, and we obtain the distinguished triangle
\[
\sgDM{k}(M', n) \to \sgDM{k}(M, n) \to \sgDM{k}(M'', n)
   \to \sgDM{k}(M', n)[1].
\]
It follows that $\sgDM{k}$ is a triangulated functor. This
