...
\ref{sect_TMS_DMeff}). To be more precise, consider the following 
definition.
...
\ref{prop_sDM_properties}, the content from first two sections 
is taken from \cite[\S 1]{HuKa}. The the extensions of the 
filtrations to $\DM$ are new.
...
$\DMeff$ such that the following is a distinguished triangle:
\begin{equation}\label{eq_slice_triangle_1}
\sgDM{n}M \to M \to M' \to \sgDM{n}M[1].
\end{equation}
It is established in \cite[1.4(i)]{HuKa} that $M'$ is uniquely 
defined up to unique isomorphism, and that there is a 
cohomological functor $\slDM{n}$ given by $M \mapsto M'$.
The functor $\slDM{n}$ is the left adjoint to the inclusion of 
$\LFiltDM[n]{\DMeff}$ in $\DMeff$ (\cite[1.4(ii)]{HuKa}). If $M$ 
is in $\LFiltDM[n]{\DMeff}$, then $\sgDM{n} M = 
\LHI[n]{\RHI[n]{M}} = 0$. Since \eqref{eq_slice_triangle_1} is
distinguished, $M \cong \slDM{n} M$. To show that this isomorphism
is natural in $M$, we will prove the stronger result that 
\eqref{eq_slice_triangle_1} is natural in $M$.
...
dotted arrow in the diagram above) is the map $\slDM{n} f$. 
...
Setting $m = n - 1$ in \eqref{eq_slice_triangle_2} and using
Proposition \ref{prop_sDM_properties} (1), we obtain the
...
$\DMeff$ as the full subcategory of $\DM$. 
...
image of $\GFiltDM[n]{\DMeff}$ under $\spectDM$ coincide with 
$\GFiltDM[n]{\DM}$.
...
\LHI[n - n']{\sgDM{k - n}M}$ by (5) of Proposition 
\ref{prop_sDM_properties}. Hence, $(\sgDM{k - n'}M', n') \cong 
(\sgDM{k - n}M, n)$.

We show that the $\sgDM{k}$ are triangulated functors from $\DM$
