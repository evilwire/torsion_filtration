...
Kahn \cite{HuKa}. We then recall the definition of the derived
category of motives $\DM$ in Definition \ref{def_DM} and extend 
the two weak filtrations on 
$\DMeff$ to $\Z$-indexed weak filtrations on $\DM$, defined below 
in \eqref{eq_DM_slice_filt} and \eqref{eq_DM_slice_cofilt}. Aside from Lemmas 
...
this functor the \DEF{$k$-th slice} on $\DM$. Since both 
...
Let us first consider the following result, which relate the slice 
