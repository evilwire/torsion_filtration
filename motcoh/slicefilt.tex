...
Recall that for a subcategory $\Cat{C}$ of $\Cat{D}$,
...
\]
together with coreflection functors $\phi_i: \Cat{A} \to 
...
is \DEF{trivial} if $\Cat{A}_n$ consist of only the zero object.
\end{defn}
...
\begin{defn}[Taken from \cite{MVW} 14.1]
...
We show that $\sgDM{k}$ are the triangulated functors from $\DM$
