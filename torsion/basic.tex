...
For the remainder of the chapter, let $\Cat{A}$ be a well-powered 
abelian category, such that all small sums exists in $\Cat{A}$.
...
For any full subcategory $\Cat{C}$ of $\Cat{A}$, closed under sums 
...
\begin{enumerate}
\item A \DEF{quotient functor} is an endofunctor $\phi : \Cat{C} 
\to \Cat{C}$ together with a natural epimorphism $\eta: \id \to 
...
$A$ in $\Cat{C}$, $\phi$ applied to the kernel of the epimorphism 
$A \to \phi(A)$ is $0$.
...
Let $\phi: \Cat{C} \to \Cat{C}$ be an endofunctor of an abelian 
...
$\Cat{C}$ is idempotent. In particular, any coradical is 
idempotent (\CF \cite[I2.2]{BJV}).
\end{prop}
\begin{proof}
Fix $A$ in $\Cat{C}$, and let $\eta$ denote the natural epimorphism 
...
show that $\kappa(A/\kappa(A)) = 0$ for all $A$ in $\Cat{C}$. Fix 
...
\begin{prop}
Let $\phi$ be a pre-coradical of an abelian category $\Cat{C}$. Suppose $B$ is a 
...
\begin{defn}
Let $\Cat{C}$ be an abelian category. A \emph{torsion theory} for
$\Cat{C}$ is a pair $(\Cat{T}, \Cat{F})$ of full subcategories,
called the \emph{torsion subcategory} and the \emph{torsion-free 
subcategory} respectively, where the objects of $\Cat{T}$ are
the objects $T$ such that $\hom_{\Cat{C}}(T, F) = 0$ for every $F$
in $\Cat{F}$ and the objects of $\Cat{F}$ are the objects $F$ such 
that $\hom_{\Cat{C}}(T, F) = 0$ for every object $T$ in $\Cat{T}$.
\end{defn}

Certainly $0 \in \Cat{T} \cap \Cat{F}$. Therefore, neither 
subcategory is empty. We also have the following characterization
of the torsion and torsionfree subcategories. Recall our assumption
that $\Cat{A}$ is a well-powered abelian category, closed under
arbitrary sums.

\begin{prop}\label{prop_tt_properties}
Suppose $\Cat{T}$ and $\Cat{F}$ are two full subcategories. Then 
...
classes of idempotent pre-coradicals and torsion theories on 
