...
\item A quotient functor $\phi$ is a \DEF{coradical} if for all
...
Let $\Cat{A}$ be a well-powered Abelian category. Then every 
coradical $\phi$ gives rise to a torsion theory on $\Cat{A}$ 
defined by:
\begin{align*}
...
Dually, every radical $\kappa$ gives rise to a torsion theory
on $\Cat{A}$ defined by
\begin{align*}
...
In the case that $\kappa$ is the radical associated to the
coradical $\phi$ (see Prop. \ref{prop_rad_eq_corad}), then the
two torsion theories agree.
\end{thm}

The converse of Theorem \ref{thm_corad_implies_tt} need not hold. 
