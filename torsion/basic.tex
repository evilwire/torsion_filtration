...
\item We say that $\phi$ is \emph{idempotent} if the natural 
...
Notice that quotient subobject functors are dual notions. So are 
(pre)-radical and (pre)-coradicals. In fact, all the results in 
this section for coradicals have an corresponding dual, i.e. a 
parallel statement for radicals. Throughout this section, we 
mostly consider statements for only (pre-)coradicals functors. We 
leave some dual statements to the reader to formulate or look up 
in \cite{DTor}.
\end{rmk}
...
\begin{lem}\label{lem_idem_precorad_eq_idem_prerad}
Let $\phi$ be an \emph{idempotent pre-coradical}, and $\eta$ be its 
corresponding natural surjection. Write $\kappa(X) = \ker (X 
\stackrel{\eta_X}{\to} \phi(X))$. Then $\kappa$ is an 
\emph{idempotent pre-radical}.

Dually, if $\psi$ is an \emph{idempotent pre-radical} with natural 
injection $\epsilon$. Writing $\gamma(X) = \cok \epsilon_X$, we 
have that $\gamma$ is an \emph{idempotent pre-coradical}.
\end{lem}
...
map. It is clear that $\kappa$ and is functorial.
...
Resuming, by Prop. \ref{prop_idempotence} and the preceding lemma,
$\phi$ (a coradical in this case) defines an idempotent 
pre-radical $\kappa$. To finish, we only need to show that

\pfitem{$\kappa$ is left exact}: Fix an injection $f: X \to Y$,
and consider the corresponding commutative diagram:
\[
\begin{tikzcd}
0 \arrow{r} &
\kappa(X) \arrow{r} \arrow{d}{\kappa f} &
X \arrow{r}{\eta_X} \arrow{d}{f} &
\phi(X) \arrow{r} \arrow{d}{\phi f} &
0 \\
0 \arrow{r} &
...
\end{tikzcd}
\]
By the Snake Lemma, $\ker \kappa f$ injects into $\ker f = 0$ 
(since $f$ is assumed to be injective). Therefore, $\ker \kappa 
f = 0$ as well. Hence $\kappa$ is left exact.
\end{proof}
