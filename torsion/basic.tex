...
neither is the methodology. We have included the proofs for 
...
A_{\Cat{C}}$ such that $jf = i$. A choice of $A_{\Cat{C}}$ for 
each $A$ determines a right adjoint to the inclusion of $\Cat{C}$ 
in $\Cat{A}$, making $\Cat{C}$ a coreflective subcategory of 
$\Cat{A}$.
\end{defn}
...
\begin{enumerate}
\item A \DEF{quotient functor} is an endofunctor $\phi : \Cat{A} \to \Cat{A}$
together with a natural surjection $\eta: \id \to \phi$. 
...
category $\Cat{A}$. We say that $\phi$ is a \DEF{pre-radical} if 
...
follows that $\cok \phi(f) = B$. The rest of the claim now 
follows.
...
K \arrow{r}{\eta_K} \arrow[equals]{d} &
